...
async def detect_topic(question: str, mode: str = None) -> str:
    ...
    # Eğer net bir eşleşme yoksa GPT'ye sor
    if max_matches <= 1:
                ...
                messages=[
                    {
                        ...
                        "content": """Kullanıcı mesajını analiz ederek aşağıdaki kategorilerden hangisine 
                                    ...
                                    4. general: Selamlaşma ("merhaba", "selam", "nasılsın"), 
                                       günlük konuşma, sohbet, genel sorular vb.
...
async def answer_question(question: str, mode: str = "real-estate", conversation_history: List = None) -> str:
    """
    Tam İyileştirilmiş Ana Fonksiyon:
    1. Gelişmiş konu tespiti yapar (150'şer kelime ile)
    ...
    print(f"🚀 TAM İYİLEŞTİRİLMİŞ SORGU BAŞLADI - Soru: {question[:50]}..., Mod: {mode}")
    ...
    # 2. GELİŞMİŞ KONU TESPİTİ (150'şer kelime ile)
    ...
    if conversation_history and len(conversation_history) > 0:
        for msg in conversation_history[-10:]:  # Son 10 mesajı al (performans için)
    ...
    # 8. AKILLI MODEL VE PARAMETRE SEÇİMİ
    try:
        print("🤖 OpenAI API'ye istek gönderiliyor...")
        
        # Model seçimi
        if mode == "real-estate" and "Bu soru için ilan araması gerekmemektedir." not in context:
            selected_model = "gpt-4o-mini"  # İlan araması için güçlü model
            temp = 0.3  # Tutarlı format için düşük
        else:
            selected_model = "gpt-4o-mini"  # Genel sorular için
            temp = 0.6  # Yaratıcı yanıtlar için orta
        ...
        print(f"✅ TAM İYİLEŞTİRİLMİŞ YANIT HAZIR - Uzunluk: {len(answer)} karakter")
...
def validate_system_configuration():
    """Sistem konfigürasyonunu doğrular"""
    issues = []
